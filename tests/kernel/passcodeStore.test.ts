...
describe("changing encryption on a live store", () => {
  it("enables, changes, and removes encryption without losing pending changes", async () => {
    ...
    await expect(
      firstCodec.decode((await adapter.read(DOCUMENT_KEY))!),
    ).rejects.toThrow();
    ...
    expect(
      JSON.parse((await adapter.read(DOCUMENT_KEY))!).modules.sleep.note,
    ).toBe("Synthetic private record");
  });
  ...
  it("keeps the previous codec and settings after a failed encryption write", async () => {
    ...
    await expect(store.setCodec(codec, true)).rejects.toThrow(
      "QuotaExceededError",
    );
