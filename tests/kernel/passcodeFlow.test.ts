...
describe("the passcode workflow", () => {
  it("sets, locks, unlocks and removes a code through Settings while retaining the document", async () => {
    ...
    async function backup(): Promise<void> {
      ...
      await vi.waitFor(() =>
        expect(root.textContent).toContain("Backup downloaded."),
      );
    }
    try {
      settings();
      ...
      await vi.waitFor(() =>
        expect(root.textContent).toContain("Passcode saved."),
      );
      expect(envelopeOf((await adapter.read(DOCUMENT_KEY))!)).not.toBeNull();
      click("Lock now");
      await vi.waitFor(() =>
        expect(root.textContent).toContain("Unlock your data"),
      );
      ...
      expect(
        JSON.parse(await unseal("123456", encrypted!)).modules.future.note,
      ).toBe("Synthetic private data");
      input("Current passcode", "123456");
      click("Remove passcode");
      await vi.waitFor(() =>
        expect(root.textContent).toContain("Passcode removed."),
      );
