...
const byText = (root: ParentNode, text: string): Element | undefined =>
  [...root.querySelectorAll("button, h1, h2, p")].find(
...
describe("tier wording", () => {
  ...
  it("names the right population in the Family space", () => {
    expect(tierWording("A", "family")).toContain(
  ...
  it("never calls a Tier B or C tool evidence-based", () => {
    ...
    expect(tierWording("B", "adult").toLowerCase()).not.toContain(
      "evidence-based",
    );
    expect(tierWording("C", "adult")).toContain(
      "techniques used in evidence-based treatment",
    );
    expect(tierWording("C", "adult")).toContain(
      "has not itself been tested in trials",
    );
...
describe("navigation once first run is done", () => {
  ...
  it("returns to the tab it came from", () => {
    ...
    expect(container.querySelector(".tabs")?.hasAttribute("hidden")).toBe(
      false,
    );
  });
  ...
  it("says what each empty tab is for, without blaming anyone", () => {
    ...
    for (const tab of TABS) {
      ...
      expect(text).not.toMatch(
        /you forgot|you missed|you have not|keep it up|streak/i,
      );
    }
  });
});
...
describe("the tab views", () => {
  ...
  it("show an empty state per tab", () => {
    expect(renderTab("today", context).textContent).toContain(
      "Nothing to fill in",
    );
    expect(renderTab("records", context).textContent).toContain(
  ...
  it("list every known module in the Library, enabled or not", () => {
    ...
    expect(view.textContent).toContain(
      "It will not diagnose a sleep disorder.",
    );
  });
  ...
  it("show only enabled modules on Today", () => {
    ...
    expect(
      renderTab("today", { space: "adult", enabled, known: enabled })
        .textContent,
    ).toContain("Sleep");
...
describe("settings", () => {
  ...
  it("keeps the unavailable-storage warning after first run and navigation", () => {
    ...
    shell.router.goTab("library");
    expect(container.textContent).toContain(
      "not letting Adnotia save anything",
    );
  ...
  it("does not claim a restore was saved when storage rejects it", async () => {
    ...
    try {
      await vi.waitFor(() => {
        expect(container.textContent).toContain(
          "The backup was merged, but could not be saved.",
        );
      });
  ...
  it("leads to backups", () => {
    ...
    click(byText(container, "Settings"));
    click(
      [...container.querySelectorAll(".linkrow")].find((r) =>
        r.textContent?.includes("Backups"),
      ),
    );
  ...
  it("persists a restored document and keeps the confirmation visible", async () => {
    ...
    try {
      ...
      expect(store.document().modules["sleep"]).toEqual(
        backup.modules["sleep"],
      );
      const reopened = createStore({ adapter });
      await reopened.load();
      expect(reopened.document().modules["sleep"]).toEqual(
        backup.modules["sleep"],
      );
