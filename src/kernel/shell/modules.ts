  ...
  const page: OffTabPage = {
    ...
    render(container) {
      ...
      const modules = options.modules
        .filter((manifest) => manifest.audience === (space === 'adult' ? 'adult' : 'parent'))
        .sort((left, right) => rank(left.id) - rank(right.id));
      container.replaceChildren();
      if (modules.length === 0 || (space === 'family' && store.profile() === undefined)) {
        container.append(el('p', { text: 'No tools available in this space yet.' }));
        return;
      }
      for (const [index, manifest] of modules.entries()) {
        ...
        const row = moduleChoice({
          ...
          onChange: (enabled) => {
            if (enabled && store.get(manifest.id) === undefined) {
              ...
              const initial =
                typeof fixture === 'object' && fixture !== null && !Array.isArray(fixture)
        ...
        remove.addEventListener('click', () => {
          ...
          if (
            !confirm(`Delete all ${manifest.name} data in this space? This cannot be undone here.`)
          )
            return;
        ...
        function move(offset: number): void {
          ...
          store.updateKernel((kernel) => ({
            ...kernel,
            moduleOrder: [...ids, ...kernel.moduleOrder.filter((id) => !ids.includes(id))],
          }));
