  ...
  const page: OffTabPage = {
    ...
    render(container) {
      ...
      download.addEventListener("click", () => {
        ...
        void exportBackup(options.store.document(), {
          ...
          .then((backup) => {
            ...
            status.textContent =
              "Backup downloaded. Your passcode has not changed.";
          })
          .catch(() => {
            status.textContent =
      ...
      async function change(removing: boolean): Promise<void> {
        ...
        if (
          !removing &&
          (!isValidPasscode(next.value()) || next.value() !== confirm.value())
        ) {
          status.textContent =
            "Enter the same passcode twice, using six or more digits.";
          return;
        }
        busy = true;
        fields.disabled = true;
