...
- Anything a browser stores can be read by a browser extension the person has given permission to read the pages they visit. This is true of every website and the app cannot detect it: no page can enumerate extensions or their permissions, and the side channels that remain find only extensions that visibly alter a page, never one that quietly reads storage. So it is stated rather than checked — on the landing page, in About, and in the first-run step that offers a passcode — and the app never implies it has looked. See `decisions/ADR-039-browser-extensions-are-said-not-detected.md`.
