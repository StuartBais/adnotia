...
**The one exception: text inside a chart.** `svg .tick` is 9 px in the chart's own coordinates, and the SVG is drawn with `width="100%"` against a 640-unit `viewBox`, so it scales with its container: about 4.8 CSS px in a report sheet on a phone, about 7.3 pt on paper. It fails both floors, and it cannot be fixed by changing the number — text that scales with the graphic cannot hold a fixed pixel floor across the roughly 2× between a phone and a printed page, because a size that clears 12.5 px on the phone is over 25 px on paper.

ADR-027 accepts it as a graphic, on the condition that nothing a chart plots is available from the picture alone: an `aria-label` describing it, a legend in body text below it, and its figures stated in words in the same section. That condition is the thing under test, not the font size. It was previously asserted in a comment and was false for the dose chart, whose printed report carried the chart and a heading while only the plain-text export named the dose levels.
